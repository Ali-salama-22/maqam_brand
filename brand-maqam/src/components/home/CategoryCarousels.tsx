...
export default function CategoryCarousels() {
  ...
  return (
    <div className="w-full flex flex-col gap-12 sm:gap-24 py-12 sm:py-20 bg-brand-bg relative z-10">
      {categories.map((category, index) => {
        ...
        return (
          <div key={category.id} className="w-full relative group">
            ...
            <div className={`max-w-[1400px] mx-auto px-3 sm:px-8 flex flex-col ${isEven ? 'lg:flex-row' : 'lg:flex-row-reverse'} items-center gap-6 sm:gap-10`}>
              ...
              <div className="lg:w-1/4 flex-shrink-0 flex flex-col items-center lg:items-start justify-center relative">
                 <div className="relative z-10">
                   <h2 className="text-3xl sm:text-4xl md:text-6xl font-black text-brand-text uppercase italic tracking-tighter leading-none mb-2 sm:mb-4" dir="rtl">
                     {category.name}
                   </h2>
                   <div className={`w-16 sm:w-20 h-1 bg-brand-accent ${isEven ? 'ml-auto lg:mr-auto lg:ml-0' : 'mr-auto lg:ml-auto lg:mr-0'} mb-3 sm:mb-6`} />
              ...
              <div className="w-full lg:w-3/4 relative">
                 <div className="flex gap-4 sm:gap-6 overflow-x-auto pb-6 sm:pb-8 pt-4 px-2 sm:px-4 snap-x snap-mandatory scrollbar-hide hide-scroll-bar" dir="rtl">
                    {categoryProducts.map((product) => (
                      <div key={product.id} className="min-w-[220px] sm:min-w-[280px] md:min-w-[320px] snap-center shrink-0">
                        <ProductCard
                          id={product.id}
                          name={product.name}
                          price={product.price}
